...
func (repo *ProductRepository) GetAll() ([]models.Product, error) {
	...
	query := "SELECT p.id, p.name, p.price, p.stock, p.category_id, c.name FROM products p LEFT JOIN categories c ON p.category_id = c.id"
...
func (repo *ProductRepository) GetByID(id int) (*models.Product, error) {
	...
	query := "SELECT p.id, p.name, p.price, p.stock, p.category_id, c.name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.id = $1"
